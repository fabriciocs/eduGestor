...
export async function listAuditEvents(tenantId: string, pagination: PaginationInput) {
  const { from, to } = toRange(pagination);
  const { data, error, count } = await supabaseAdmin
    .from('audit_events')
    .select('id, actor_user_id, action, entity_type, entity_id, created_at, metadata', { count: 'exact' })
